...
import type { HostPictureCaptionBallot, HostPictureCaptionRevealCandidate, HostPictureCaptionTemplate, PartyMemberProjection, PartyProjection, PictureCaptionCompletion, PictureCaptionRound, PictureCaptionSubmission } from "@/lib/supabase/database.types";
      ...
      <div className="dashboard-grid">
        <section className="dashboard-stage setup-stage">
          ...
          <div className="broadcast-panel setup-panel">
            ...
            {party.game_session_state === "live" && rounds.some((round) => round.phase === "revealing" || round.phase === "results") && <><div className="roster-list reveal-grid">{revealCandidates.map((candidate) => <button className="roster-row reveal-card" type="button" key={candidate.candidate_id} disabled={busy || !canWrite || candidate.revealed} onClick={() => void roundCommand({ action: "reveal", candidateId: candidate.candidate_id }, "results")}><div className="reveal-card-heading"><span className="reveal-card-letter" style={candidate.revealed && candidate.author_color ? { borderColor: candidate.author_color } : undefined}>{candidate.letter}</span><strong className="reveal-card-author">{candidate.revealed ? (candidate.is_official ? "Official" : candidate.author_nickname) : "Tap to reveal"}</strong></div><p>{candidate.caption}</p>{(candidate.voter_nicknames?.length ?? 0) > 0 && <div className="reveal-card-voters">{candidate.voter_nicknames!.map((voterNickname, index) => <span className="roster-color" key={`${candidate.candidate_id}-${voterNickname}`} style={{ backgroundColor: candidate.voter_colors?.[index] ?? undefined }} title={voterNickname} />)}</div>}</button>)}</div><button className="broadcast-action" type="button" disabled={busy || !canWrite || revealCandidates.length === 0 || revealCandidates.some((candidate) => !candidate.revealed)} onClick={() => void roundCommand({ action: "continue" }, "results")}>Continue</button></>}
