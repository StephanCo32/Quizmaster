      ...
      <div className="dashboard-grid">
        <section className="dashboard-stage setup-stage">
          ...
          <div className="broadcast-panel setup-panel">
            ...
            {party.game_session_state === "live" && rounds.some((round) => round.phase === "revealing" || round.phase === "results") && <><div className="roster-list reveal-grid">{revealCandidates.map((candidate) => <button className="reveal-card" type="button" key={candidate.candidate_id} disabled={busy || !canWrite || candidate.revealed} onClick={() => void roundCommand({ action: "reveal", candidateId: candidate.candidate_id }, "results")} style={candidate.author_colors?.[0] ? { borderColor: candidate.author_colors[0] } : undefined}><span className="reveal-card-letter">{candidate.letter}</span><span className="reveal-card-caption"><p>{candidate.caption}</p><span className="reveal-card-status">{candidate.revealed ? "Shown to the room" : "Tap to show the room"}</span></span>{(candidate.voter_nicknames?.length ?? 0) > 0 && <div className="reveal-card-voters">{candidate.voter_nicknames!.map((voterNickname, index) => <span className="roster-color" key={`${candidate.candidate_id}-${voterNickname}`} style={{ backgroundColor: candidate.voter_colors?.[index] ?? undefined }} title={voterNickname} />)}</div>}<strong className="reveal-card-author">{candidate.is_official ? ((candidate.author_nicknames?.length ?? 0) > 0 ? `Official · ${candidate.author_nicknames!.join(", ")}` : "Official") : candidate.author_nicknames?.join(", ")}</strong></button>)}</div><button className="broadcast-action" type="button" disabled={busy || !canWrite || revealCandidates.length === 0 || revealCandidates.some((candidate) => !candidate.revealed)} onClick={() => void roundCommand({ action: "continue" }, "results")}>Continue</button></>}
