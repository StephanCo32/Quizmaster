      ...
      <div className="dashboard-grid">
        <section className="dashboard-stage setup-stage">
          ...
          <div className="broadcast-panel setup-panel">
            ...
            {party.game_session_state === "live" && rounds.some((round) => round.phase === "revealing") && <><div className="roster-list">{revealCandidates.map((candidate) => <button className="roster-row" type="button" key={candidate.candidate_id} disabled={busy || !canWrite || candidate.revealed} style={candidate.revealed && candidate.author_color ? { backgroundColor: candidate.author_color } : undefined} onClick={() => void roundCommand({ action: "reveal", candidateId: candidate.candidate_id }, "results")}><div><strong>{candidate.letter}</strong><span>{candidate.revealed ? `${candidate.is_official ? "Official" : candidate.author_nickname} · ${candidate.caption}` : "Tap to reveal"}</span></div></button>)}</div><button className="broadcast-action" type="button" disabled={busy || !canWrite || revealCandidates.length === 0 || revealCandidates.some((candidate) => !candidate.revealed)} onClick={() => void roundCommand({ action: "continue" }, "results")}>Continue</button></>}
