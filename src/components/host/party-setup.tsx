...
import type { HostPictureCaptionBallot, HostPictureCaptionCompletion, HostPictureCaptionRevealCandidate, HostPictureCaptionTemplate, PartyMemberProjection, PartyProjection, PictureCaptionRound, PictureCaptionSubmission } from "@/lib/supabase/database.types";
...
export function PartySetup({
  ...
  initialCompletion: HostPictureCaptionCompletion | null;
  initialBallots: HostPictureCaptionBallot[];
  initialRevealCandidates: HostPictureCaptionRevealCandidate[];
}) {
  ...
  async function refresh() {
    ...
    const captions = await captionsResponse.json() as { submissions: PictureCaptionSubmission[]; completion: HostPictureCaptionCompletion | null };
    ...
    const revealResponse = await fetch(`/api/host/parties/${party.party_id}/results`, { cache: "no-store" });
    if (!revealResponse.ok) throw new Error("reveal_projection_unavailable");
    setRevealCandidates((await revealResponse.json() as { candidates: HostPictureCaptionRevealCandidate[] }).candidates);
  }
  ...
  async function roundCommand(body: Record<string, unknown>, path = "rounds") {
    ...
    if (!response.ok) {
      ...
      setError(payload?.error === "official_caption_required" ? "That template is missing an official caption. Add one from the content desk first." : response.status === 409 ? "The session changed. Refresh and try again." : "The round command could not be saved.");
    }
    else await refresh();
    setBusy(false);
  }
      ...
      <div className="dashboard-grid">
        <section className="dashboard-stage setup-stage">
          ...
          <div className="broadcast-panel setup-panel">
            ...
            {party.game_session_state === "live" && rounds.some((round) => round.state === "active" && (round.phase === "captioning" || round.phase === "voting")) && <button className="broadcast-action" type="button" disabled={busy || !canWrite} onClick={() => void roundCommand({ paused: rounds.find((round) => round.state === "active")?.captioning_deadline !== null || rounds.find((round) => round.state === "active")?.turn_deadline !== null }, "session/pause")}>{(rounds.find((round) => round.state === "active")?.captioning_deadline || rounds.find((round) => round.state === "active")?.turn_deadline) ? <Pause aria-hidden="true" /> : <Play aria-hidden="true" />} {(rounds.find((round) => round.state === "active")?.captioning_deadline || rounds.find((round) => round.state === "active")?.turn_deadline) ? "Pause timer" : "Resume timer"}</button>}
            {party.game_session_state === "live" && rounds.some((round) => round.phase === "captioning") && <><p>{completion?.submission_count ?? 0} of {completion?.eligible_count ?? 0} captions submitted</p>{(completion?.missing_nicknames?.length ?? 0) > 0 && <p className="missing-captions">Waiting on: {completion!.missing_nicknames!.join(", ")}</p>}<div className="roster-list">{submissions.map((submission) => <div className="roster-row" key={submission.submission_id}><div><strong>{submission.nickname}</strong><span>{submission.caption}</span></div><button className="icon-button" type="button" title={`Remove ${submission.nickname}'s caption`} disabled={busy || !canWrite} onClick={() => void captionCommand({ action: "remove", submissionId: submission.submission_id })}><Trash2 aria-hidden="true" /></button></div>)}</div><button className="broadcast-action" type="button" disabled={busy || !canWrite} onClick={() => setConfirmation({ title: "Close Captioning", message: "Close Captioning even if responses are missing?", confirmLabel: "Close Captioning", onConfirm: () => void captionCommand({ action: "close", confirmMissing: true }) })}>Close Captioning</button></>}
            {party.game_session_state === "live" && rounds.some((round) => round.phase === "voting") && <><p>{rounds.find((round) => round.phase === "voting")?.current_turn_nickname ? `${rounds.find((round) => round.phase === "voting")?.current_turn_nickname}'s turn` : "Voting"}</p><div className="roster-list">{ballots.map((ballot) => <div className="roster-row" key={`${ballot.candidate_id}-${ballot.voter_nickname ?? "pending"}`}><div><strong>{ballot.caption}</strong><span>{ballot.voter_nickname ?? "No ballot yet"}</span></div><b>{ballot.points}</b></div>)}</div><button className="broadcast-action" type="button" disabled={busy || !canWrite} onClick={() => void roundCommand({}, "ballots")}>Force-skip turn</button></>}
            {party.game_session_state === "live" && rounds.some((round) => round.phase === "revealing" || round.phase === "results") && <><div className="roster-list reveal-grid">{revealCandidates.map((candidate) => prototypeVariant ? <RevealCard key={candidate.candidate_id} variant={prototypeVariant} candidate={candidate} disabled={busy || !canWrite} onReveal={() => void roundCommand({ action: "reveal", candidateId: candidate.candidate_id }, "results")} /> : <button className="roster-row reveal-card" type="button" key={candidate.candidate_id} disabled={busy || !canWrite || candidate.revealed} onClick={() => void roundCommand({ action: "reveal", candidateId: candidate.candidate_id }, "results")}><div className="reveal-card-heading"><span className="reveal-card-letter" style={candidate.author_colors?.[0] ? { borderColor: candidate.author_colors[0] } : undefined}>{candidate.letter}</span><strong className="reveal-card-author">{candidate.is_official ? "Official" : candidate.author_nicknames?.join(", ")}</strong></div><p>{candidate.caption}</p>{(candidate.voter_nicknames?.length ?? 0) > 0 && <div className="reveal-card-voters">{candidate.voter_nicknames!.map((voterNickname, index) => <span className="roster-color" key={`${candidate.candidate_id}-${voterNickname}`} style={{ backgroundColor: candidate.voter_colors?.[index] ?? undefined }} title={voterNickname} />)}</div>}<span className="reveal-card-status">{candidate.revealed ? "Shown to the room" : "Tap to show the room"}</span></button>)}</div><button className="broadcast-action" type="button" disabled={busy || !canWrite || revealCandidates.length === 0 || revealCandidates.some((candidate) => !candidate.revealed)} onClick={() => void roundCommand({ action: "continue" }, "results")}>Continue</button></>}
            {party.game_session_state === "live" && !rounds.some((round) => round.state === "active") && <div className="round-lifecycle-actions">{rounds.some((round) => round.awaiting_reveal) && <button className="broadcast-action" type="button" disabled={busy || !canWrite} onClick={() => void roundCommand({ action: "start-reveal" }, "results")}>Start Reveal</button>}{!rounds.some((round) => round.awaiting_reveal) && rounds.some((round) => round.state === "pending") && <button className="broadcast-action" type="button" disabled={busy || !canWrite} onClick={() => void roundCommand({}, "session/start-next")}>Start next round</button>}<button className="broadcast-action" type="button" disabled={busy || !canWrite} onClick={() => void lifecycleCommand({ action: "finish" })}>Finish Game session</button></div>}
