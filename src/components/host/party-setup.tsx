...
import type { PartyMemberProjection, PartyProjection } from "@/lib/supabase/database.types";

export function PartySetup({ party: initialParty, roster: initialRoster }: { party: PartyProjection; roster: PartyMemberProjection[] }) {
    const [party, setParty] = useState(initialParty);
    const [roster, setRoster] = useState(initialRoster);

    async function refresh() {
        const response = await fetch(`/api/host/parties/${party.party_id}/lobby`, { cache: "no-store" });
        if (!response.ok) throw new Error("host_projection_unavailable");
        const projection = await response.json() as { party: PartyProjection; roster: PartyMemberProjection[] };
        setParty(projection.party);
        setRoster(projection.roster);
    }

    const connectionState = useLobbySynchronization({ gameSessionId: party.game_session_id, revision: party.revision, refetch: refresh });
        ...
        <main className="broadcast-shell">
            ...
            <div className="dashboard-grid">
                <section className="dashboard-stage setup-stage">
                    ...
                    {connectionState !== "connected" && <div className="status-notice status-error" role="status"><WifiOff aria-hidden="true" /> {connectionState === "reconnecting" ? "Reconnecting and refreshing the committed Lobby..." : "Disconnected. Showing the last committed Lobby; Host writes are paused."}</div>}
                    <div className="broadcast-panel setup-panel">
                        <div className="panel-heading">
                            <SlidersHorizontal aria-hidden="true" />
                            <div><span>Current Game session</span><h2>{party.game_session_state}</h2></div>
                        </div>
                            ...
                            {party.game_session_state === "setup" && <OpenLobbyButton partyId={party.party_id} expectedRevision={party.revision} disabled={!canWrite} onOpened={refresh} />}
                    </div>
                </section>
