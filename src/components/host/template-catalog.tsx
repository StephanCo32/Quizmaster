...
export function TemplateCatalog({ initialTemplates }: { initialTemplates: PictureCaptionTemplate[] }) {
    ...
    function resetForm() { setEditing(null); setCreating(false); setForm({ name: "", pictureUrl: "", prompt: "" }); }
    function editTemplate(template: PictureCaptionTemplate) { setCreating(false); setEditing(template); setForm({ name: template.name, pictureUrl: template.picture_url, prompt: template.prompt ?? "" }); setError(null); }
    ...
    return (
        <main className="broadcast-shell">
            ...
            <div className="dashboard-grid"><section className="dashboard-stage"><p className="broadcast-kicker">Content desk</p><div className="dashboard-title-row"><div><h1>Picture-caption rounds</h1><p>Maintain the image and prompt library for new games.</p></div><button className="broadcast-action" type="button" onClick={() => { resetForm(); setCreating(true); setError(null); }}><Plus aria-hidden="true" /> New template</button></div>
