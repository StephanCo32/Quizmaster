    ...
    <main className="broadcast-shell">
      ...
      <div className="dashboard-grid">
        <section className="dashboard-stage">
          ...
          {activeRound && <section className="broadcast-panel player-card">{activeRound.phase === "captioning" && <Image src={`/api/play/${partyCode}/rounds/${activeRound.round_id}/picture`} alt="Picture caption round" width={640} height={360} unoptimized />}<div><span className="rail-label">{activeRound.phase}{activeRound.paused_remaining_seconds !== null ? " paused" : ""}</span><h2>{activeRound.phase === "voting" ? "Choose your favorite caption." : activeRound.official_caption ?? "Write a caption."}</h2>{secondsRemaining !== null && <p className="round-countdown">{secondsRemaining}s</p>}</div></section>}
