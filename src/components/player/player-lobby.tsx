    ...
    <main className="broadcast-shell">
      ...
      <div className="dashboard-grid">
        <section className="dashboard-stage">
          ...
          {activeRound?.phase === "captioning" && <form className="broadcast-panel nickname-form caption-form" onSubmit={submitCaption}><label>Caption<textarea required maxLength={1000} rows={3} value={caption} onChange={(event) => setCaption(event.target.value)} /></label><button className="broadcast-action" type="submit" disabled={busy || !canWrite || activeRound.captioning_deadline === null}>{caption ? "Update caption" : "Submit caption"}</button></form>}
