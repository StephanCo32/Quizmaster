...
export function DisplayOverview({ initialParty, initialRoster, initialActiveRound }: { initialParty: DisplayPartyProjection; initialRoster: DisplayMemberProjection[]; initialActiveRound: ActivePictureCaptionRound | null }) {
    ...
    async function refresh() {
        ...
        const projection = await response.json() as { canonicalCode: string; party: DisplayPartyProjection; roster: DisplayMemberProjection[]; activeRound: ActivePictureCaptionRound | null };
    ...
    return <main className="display-overview"><header className="display-overview-header"><span className="broadcast-brand"><span className="broadcast-mark">Q</span><span>Quizmaster</span></span><span className="signal-chip"><Radio size={16} /> Party {party.party_code}</span><span>{party.game_session_state}</span></header>{activeRound && <section className="display-scoreboard"><Image src={`/api/display/${party.party_code}/rounds/${activeRound.round_id}/picture`} alt="Picture caption round" width={960} height={540} unoptimized /><div><span className="broadcast-kicker">{activeRound.phase}{activeRound.paused_remaining_seconds !== null ? " paused" : ""}</span><h1>{activeRound.prompt ?? "Write a caption."}</h1><span>{activeRound.captioning_deadline ? `Ends ${new Date(activeRound.captioning_deadline).toLocaleTimeString()}` : `${activeRound.paused_remaining_seconds ?? 0} seconds remaining`}</span></div></section>}<section className="display-scoreboard"><div><span className="broadcast-kicker">Shared Lobby</span><h1>{roster.length} Players</h1></div><div className="display-connection" role="status">{canWriteLobby(connectionState) ? "Live" : <><WifiOff aria-hidden="true" /> {connectionState === "reconnecting" ? "Reconnecting" : "Disconnected"}</>}</div></section><section className="display-roster" aria-label="Player roster">{roster.map((member) => <article className="display-member" key={member.member_id}><span className="roster-color" style={{ backgroundColor: member.color }} aria-hidden="true" /><strong>{member.nickname}</strong><span>{member.ready ? "Ready" : "Waiting"}</span><b>{member.score}</b></article>)}</section></main>;
}