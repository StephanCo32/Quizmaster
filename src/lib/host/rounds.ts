...
import type { HostPictureCaptionBallot, HostPictureCaptionCompletion, HostPictureCaptionRevealCandidate, HostPictureCaptionTemplate, LobbyCommandResult, PictureCaptionRound, PictureCaptionSubmission } from "@/lib/supabase/database.types";
...
export async function getHostPictureCaptionCompletion(hostId: string, partyId: string): Promise<HostPictureCaptionCompletion | null> { const { data, error } = await createSupabaseAdminClient().rpc("host_picture_caption_completion_projection", { p_host_id: hostId, p_party_id: partyId }); if (error) throw new Error("caption_projection_unavailable", { cause: error }); return data.at(0) ?? null; }
...
export async function forceSkipPictureCaptionTurn(command: { hostId: string; partyId: string; commandId: string; expectedRevision: number }): Promise<LobbyCommandResult> { const { data, error } = await createSupabaseAdminClient().rpc("force_skip_picture_caption_turn", { p_host_id: command.hostId, p_party_id: command.partyId, p_command_id: command.commandId, p_expected_revision: command.expectedRevision }); if (error) throw new Error(error.code === "40001" ? error.message : "turn_skip_failed", { cause: error }); const result = data.at(0); if (!result) throw new Error("turn_skip_failed"); return result; }
export async function getHostPictureCaptionRevealCandidates(hostId: string, partyId: string): Promise<HostPictureCaptionRevealCandidate[]> { const { data, error } = await createSupabaseAdminClient().rpc("host_picture_caption_reveal_projection", { p_host_id: hostId, p_party_id: partyId }); if (error) throw new Error("reveal_projection_unavailable", { cause: error }); return data; }
export async function revealPictureCaptionCandidate(command: { hostId: string; partyId: string; commandId: string; expectedRevision: number; candidateId: string }): Promise<LobbyCommandResult> { const { data, error } = await createSupabaseAdminClient().rpc("reveal_picture_caption_candidate", { p_host_id: command.hostId, p_party_id: command.partyId, p_command_id: command.commandId, p_expected_revision: command.expectedRevision, p_candidate_id: command.candidateId }); if (error) throw new Error(error.code === "40001" ? "stale_revision" : error.message === "candidate_not_found" ? "candidate_not_found" : "reveal_failed", { cause: error }); const result = data.at(0); if (!result) throw new Error("reveal_failed"); return result; }
