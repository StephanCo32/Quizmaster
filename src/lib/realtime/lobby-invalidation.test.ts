...
const { channel, createSupabaseAdminClient, httpSend, removeChannel } = vi.hoisted(() => {
    ...
    const createSupabaseAdminClient = vi.fn(() => ({ channel: vi.fn(() => channel), removeChannel }));
...
import { publishLobbyInvalidation } from "./lobby-invalidation";
...
describe("Lobby invalidations", () => {
    ...
    it("keeps a committed command successful when Broadcast is unavailable", async () => {
        httpSend.mockRejectedValue(new Error("unavailable"));
        const error = vi.spyOn(console, "error").mockImplementation(() => undefined);
        await expect(publishLobbyInvalidation({ gameSessionId: "session-1", revision: 4 })).resolves.toBe(false);
