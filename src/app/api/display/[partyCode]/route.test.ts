...
const { getDisplayParty, getDisplayPartyLobby, getDisplaySessionId } = vi.hoisted(() => ({
    getDisplayParty: vi.fn(),
    getDisplayPartyLobby: vi.fn(),
    getDisplaySessionId: vi.fn(),
}));

vi.mock("@/lib/display/sessions", () => ({ getDisplayParty, getDisplayPartyLobby, getDisplaySessionId }));
...
describe("GET /api/display/[partyCode]", () => {
    ...
    it("returns only the authorized Display projection", async () => {
        ...
        await expect(response.json()).resolves.toEqual(expect.objectContaining({ roster: [expect.objectContaining({ nickname: "Ada" })] }));
    });
});