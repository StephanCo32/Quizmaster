...
import { changeNickname, getPlayerPartyCanonicalCode, getPlayerPartyLobby, getPlayerPictureCaptionCandidates, getPlayerPictureCaptionResults, getPlayerPictureCaptionRound, getPlayerPictureCaptionSubmission, setReady } from "@/lib/player/parties";
...
export async function GET(_request: Request, context: { params: Promise<{ partyCode: string }> }) {
    ...
    try {
        ...
        const [roster, activeRound, submission, candidates, results] = await Promise.all([getPlayerPartyLobby(playerId, canonicalCode), getPlayerPictureCaptionRound(playerId, canonicalCode), getPlayerPictureCaptionSubmission(playerId, canonicalCode), getPlayerPictureCaptionCandidates(playerId, canonicalCode), getPlayerPictureCaptionResults(playerId, canonicalCode)]);
        return NextResponse.json({ canonicalCode, roster, activeRound, submission, candidates, results });
    }
