...
const updateSchema = z.object({ commandId: z.string().uuid(), expectedRevision: z.number().int().nonnegative(), name: z.string().trim().min(1).max(100), pictureUrl: z.string().url().refine((value) => value.startsWith("https://")), prompt: z.string().max(280).nullable().optional() });
...
export async function PUT(request: Request, context: { params: Promise<{ templateId: string }> }) {
    ...
    try { return NextResponse.json({ template: await updatePictureCaptionTemplate({ adminId: admin.id, templateId, ...parsed.data, prompt: parsed.data.prompt ?? null }) }); }
