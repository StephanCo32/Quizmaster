...
const updateSchema = z.object({ commandId: z.string().uuid(), expectedRevision: z.number().int().nonnegative(), name: z.string().trim().min(1).max(100), pictureUrl: z.string().url().refine((value) => value.startsWith("https://")), officialCaption: z.string().trim().min(1).max(280) });
...
export async function PUT(request: Request, context: { params: Promise<{ templateId: string }> }) {
    ...
    try { return NextResponse.json({ template: await updatePictureCaptionTemplate({ adminId: admin.id, templateId, ...parsed.data }) }); }
    catch (error) { const message = error instanceof Error ? error.message : "unavailable"; return NextResponse.json({ error: message === "official_caption_required" ? message : message === "stale_revision" ? message : "unavailable" }, { status: message === "official_caption_required" ? 400 : message === "stale_revision" ? 409 : 503 }); }
}
