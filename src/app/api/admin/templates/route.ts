...
const createSchema = z.object({ commandId: z.string().uuid(), name: z.string().trim().min(1).max(100), pictureUrl: z.string().url().refine((value) => value.startsWith("https://")), prompt: z.string().max(280).nullable().optional() });
...
export async function POST(request: Request) {
    ...
    try { return NextResponse.json({ template: await createPictureCaptionTemplate({ adminId: admin.id, commandId: parsed.data.commandId, name: parsed.data.name, pictureUrl: parsed.data.pictureUrl, prompt: parsed.data.prompt ?? null }) }, { status: 201 }); }
